...
const AUTH_CACHE_TTL_MS = 15000; // 15 seconds TTL
 ...
 * Uses a 15-second in-memory server cache to avoid duplicate DB lookups on back-to-back requests,
 * while verifying JWT token signature/expiration on every request and re-validating against MongoDB after 15s.
 */
export async function getAuthUser() {
  try {
    ...
    // 1. ALWAYS verify JWT signature & expiration on EVERY request first
    const decoded = verifyToken(token);
    if (!decoded || !decoded.id) return null;

    // 2. Check 15-second in-memory server cache
    const now = Date.now();
    ...
    // 3. Cache miss or expired (>15s): query MongoDB
    ...
    // 4. Update cache entry with current timestamp
...
export function validateCreationPermission(creatorRole, targetRole) {
  if (creatorRole === 'ADMIN') {
    return ['COORDINATOR', 'SUPERVISOR', 'DIGITAL_OPD_AGENT'].includes(targetRole);
  }
