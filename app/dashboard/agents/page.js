...
export default function ManageAgentsPage() {
  ...
  const fetchActiveSupervisors = async () => {
    try {
      const res = await fetch('/api/users?role=SUPERVISOR&status=ACTIVE');
  ...
  const handleToggleStatus = async (userToUpdate) => {
    ...
    try {
      ...
      if (data.success) {
        setAgents((prev) =>
          prev.map((a) => (a._id === userToUpdate._id ? { ...a, status: newStatus } : a))
        );
      } else {
                  ...
                  <tr key={a._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3.5 font-semibold text-slate-800 dark:text-slate-200">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full bg-emerald-500/10 border border-emerald-500/30 overflow-hidden flex items-center justify-center shrink-0">
                          {a.photoUrl ? (
                            <img src={a.photoUrl} alt={a.name} className="w-full h-full object-cover" />
                          ) : (
                            <span className="font-bold text-emerald-600 text-xs">{a.name?.[0]?.toUpperCase() || 'A'}</span>
                          )}
                        </div>
                        <span>{a.name}</span>
                      </div>
                ...
                {authUser?.role === 'ADMIN' && (
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                      Assign under Active Supervisor <span className="text-rose-500">*</span>
                    </label>
                    <select
                      required
                      value={parentId}
                    ...
                    {supervisors.length === 0 && (
                      <p className="text-[11px] text-rose-500 mt-1">
                        No active Supervisors found. Please activate a Supervisor first before creating a Digital OPD Agent under them.
                      </p>
                    )}
                  </div>
                )}
